...
(function(){
  ...
  var chart = function(selection) {
    return selection.each(function(data) {
      ...
      data.forEach(function(d) {
        ...
        d.books = Object.keys(d).map(function(name) {
          if (name != "year") {
            return {name: d[name], agent: d[name].artist, imageURI: calisphereUriPath + imgArkStore[d[name].imageURI], selected: d[name].selected, y0: y0, y1: y0 += bookHeight};
          }
          else {
            ...
            return {name: d[name], selected: "null", y0: -2000, y1: -2000};
          }
        });
      });
    ...
    // lib/images/book.svg <- TODO: make uri conditional!!
