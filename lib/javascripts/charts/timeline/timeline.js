...
(function(){
  ...
  var chart = function(selection) {
    return selection.each(function(data) {
      ...
      year.selectAll("rect")
        ...
        .enter().append("rect")
          ...
          .attr("y", function(d) {
            return tly(d.y1); })
          // adds each additional stacked item to most recent item in col
          ...
          .style("fill", function(d) {
            if (chart.selectedKey === d.key && d.selected === true) {
              return "#f36f23";
          ...
          .on('click', function(d) {
            console.log(this);
            if (chart.selectedKey.length > 0) {
              d3.select("#" + chart.selectedKey)
                .style('fill', function(d){
                  if (d.selected === true) {
                    return "#555759";
                  } else {
    ...
    tipTemplate = function(d) {
      if (d.null !== true ) {
        if (d.imageURI) {
          return "<div class='tip-agent' style='color:whitesmoke'>" + d.agent + "</div><div class='tip-content'><img alt='Photograph featuring "+d.name+"' class='d3-tip-img' src=" + d.imageURI + "><span class='tip-title' style='color:whitesmoke'>" + d.name + "</span></div><br>";
        } else {
          return "<div class='tip-agent' style='color:whitesmoke'>" + d.agent + "</div><div class='tip-content'><img alt='book icon' class='d3-tip-img' src='lib/images/book.svg'><span class='tip-title' style='color:whitesmoke'>" + d.name + "</span></div><br>";
        }
      } else {
