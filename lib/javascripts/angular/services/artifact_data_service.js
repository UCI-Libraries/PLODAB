(function() {
  angular.module('plodab')
    ...
    this.getArtifactData = function(callback, data) {
      ...
      var calisphereImageURIPath = "https://thumbnails.calisphere.org/clip/250x250/";

      var calisphereURIPath = "http://calisphere-test.cdlib.org/item/";
      ...
      function makeArtifactData(data) {
        ...
        keys.forEach( function(key) {
          ...
          artifactStore[key].typeOfWork = "Artists' Book";
          artifactStore[key].imageURI = calisphereImageURIPath + imgArkStore[data[key].imageURI];
          artifactStore[key].calisphereURI = calisphereURIPath + data[key].imageURI;
          artifactStore[key].height = 280;
          artifactStore[key].width = 800;
          artifactStore[key].level = "top";
          artifactStore[key].size = 90;
          artifactStore[key].agents = sourceAgents(data[key]);
      ...
      function sourceAgents(book) {
        ...
        keys.forEach(function(key) {
          ...
          if (agentObj.role.length > 0) {
            agentsStr = agentsStr + name + " ("+roles+"), ";
          } else if (name !== undefined){
            agentsStr = agentsStr + name + ", ";
          }
          if (agentsStore[name]) {
        ...
        // return agentsArr;
        return agentsStr.slice(0, agentsStr.length - 2);
      }
