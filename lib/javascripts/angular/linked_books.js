  ...
  angular.module('plodab')
    .controller('chartctrl', ['$scope', '$timeout', 'dataService', 'timelineDataService', 'bubbleDataService', 'artifactDataService', function ($scope, $timeout, dataService, timelineDataService, bubbleDataService, artifactDataService) {
      ...
      $scope.resetBubbleChartFromTree = function() {
        // console.log($scope.selectedTreeTopic);
        // console.log("tree change", $scope.selectedNodes, $scope.selectedTopics);
        $scope.selectedTopics = [$scope.selectedTreeTopic.topic];
        $scope.selectedNodes[$scope.selectedTreeTopic.topic] = [$scope.selectedTreeTopic.name];
        // setBubbleChartData();
        // pass selection to bubble chart and timeline respectively.
        // setTimelineData();
        // changeBubbleChartWithSelection($scope.selectedTreeTopic);
        // $scope.$apply();
      ...
      $scope.$watch('selectedNodes', function(newValue, oldValue) {
              if (newValue) {
                  // console.log("node change", $scope.selectedNodes);
                  setTimelineData();
                }
            }, true);
      ...
      function updateTimelineData() {
        console.log("updating TL", $scope.selectedNodes);
      ...
      function checkTimelineSelections(key, booksByYear) {
        ...
        // console.log("why isn't this working?", $scope.selectedNodes);
        topics.forEach( function(topic) {
          if ($scope.selectedNodes[topic].length > 0) {
            var arr = $scope.selectedNodes[topic];
            console.log("Yes, a topic", arr);
